      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        {cards.filter(c => c.status !== "dismissed").map(card => (
          <div key={card.id} style={{ display: "flex", alignItems: "center", gap: 16, padding: "12px 18px", borderRadius: 8, background: card.status === "approved" ? C.greenBg : C.amberBg }}>
            <div style={{ width: 160 }}>
              <div style={{ fontSize: 13, fontWeight: 700, color: C.text }}>{card.client}</div>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              ...
              {card.status !== "dismissed" && (
                <button onClick={() => onUpdate(card.id, { status: "dismissed" })} style={{ width: 28, height: 28, borderRadius: "50%", border: "none", background: "transparent", cursor: "pointer", fontSize: 16, color: C.textDim, display: "flex", alignItems: "center", justifyContent: "center" }} onMouseEnter={(e) => (e.currentTarget.style.color = C.red)} onMouseLeave={(e) => (e.currentTarget.style.color = C.textDim)}>✕</button>
              )}
            </div>
          </div>
        ))}
