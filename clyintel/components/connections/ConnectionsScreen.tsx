...
export default function ConnectionsScreen() {
  ...
  const svcIcon = (svc: InvoiceService, size = 32) => (
    <>
      ...
      <span style={{ fontSize: 16, fontWeight: 700, color: "#fff", display: svc.logo ? "none" : "inline" }}>
        {svc.initial}
      </span>
    </>
  );
  ...
  const backBtn = (onClick: () => void, label = "Back") => (
    <button onClick={onClick} style={{ display: "flex", alignItems: "center", gap: 6, padding: "8px 12px", fontSize: 16, fontWeight: 600, color: C.blue, background: "transparent", border: "none", cursor: "pointer", marginBottom: 12 }} onMouseEnter={e => e.currentTarget.style.textDecoration = "underline"} onMouseLeave={e => e.currentTarget.style.textDecoration = "none"}>
  ...
  const driveConnectedBadge = () => (
    <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16 }}>
      ...
      <div style={{ fontSize: 15, color: C.textMid, fontWeight: 500 }}>
        ...
        {selectedAccount && <><span style={{ color: C.border, margin: "0 8px" }}>·</span><span style={{ fontFamily: C.mono, fontSize: 15 }}>{selectedAccount.email}</span></>}
    ...
    <div style={{ padding: "36px 48px", minHeight: 520, fontFamily: C.sans }}>
        ...
        <>
          ...
          <div style={{ maxWidth: 860, margin: "0 auto", textAlign: "center" }}>
                ...
                {integrations.map(svc => (
                  <div key={svc.id} onClick={() => handleServiceClick(svc)} style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 12, padding: "28px 20px", cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "center", gap: 14, textAlign: "center", transition: "border-color 0.15s" }} onMouseEnter={e => { e.currentTarget.style.borderColor = C.blue; e.currentTarget.style.boxShadow = `0 0 0 3px ${C.blueBg}`; }} onMouseLeave={e => { e.currentTarget.style.borderColor = C.border; e.currentTarget.style.boxShadow = "none"; }}>
                    ...
                    <div>
                      ...
                      <div style={{ fontSize: 14, color: C.textDim, marginTop: 4, fontWeight: 500 }}>{svc.subtitle}</div>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: 36 }}>
              ...
              <div style={{ fontSize: 14, color: C.textDim, fontWeight: 500 }}>or</div>
              <div style={{ flex: 1, height: 1, background: C.border }} />
            </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 16 }}>
              {bottomRow.map(svc => (
                <div key={svc.id} onClick={() => handleServiceClick(svc)} style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 12, padding: "24px 20px", cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "center", gap: 12, textAlign: "center", transition: "border-color 0.15s" }} onMouseEnter={e => { e.currentTarget.style.borderColor = C.blue; e.currentTarget.style.boxShadow = `0 0 0 3px ${C.blueBg}`; }} onMouseLeave={e => { e.currentTarget.style.borderColor = C.border; e.currentTarget.style.boxShadow = "none"; }}>
                  ...
                  <div>
                    <div style={{ fontSize: 16, fontWeight: 600, color: C.text }}>{svc.name}</div>
                    <div style={{ fontSize: 14, color: C.textDim, marginTop: 4, lineHeight: 1.4, fontWeight: 500 }}>{svc.subtitle}</div>
                  </div>
                </div>
              ))}
            </div>
      ...
      {stage === "connecting" && selectedService && (
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: 360 }}>
          ...
          <div style={{ fontSize: 15, color: C.textMid, fontWeight: 500, marginBottom: 24 }}>Syncing invoice history</div>
          {progressBar("1.6s")}
        </div>
      )}
      ...
      {stage === "select" && selectedService && (
        <>
          <div style={{ marginBottom: 24 }}>
            ...
            <div style={{ fontSize: 15, color: C.textMid, fontWeight: 500 }}>Choose one or more clients to analyze and score.</div>
          </div>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 12, maxWidth: 580, marginTop: 16 }}>
            <button
              ...
              style={{ padding: "9px 18px", fontSize: 15, fontWeight: 600, color: C.textMid, background: "transparent", border: `1px solid ${C.border}`, borderRadius: 6, cursor: "pointer" }}
            ...
            {selectedClients.length > 0 && (
              <span style={{ fontSize: 15, color: C.textMid, fontWeight: 500, flex: 1 }}>
      ...
      {stage === "analyzing" && (
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: 360 }}>
          ...
          <div style={{ fontSize: 15, color: C.textMid, fontWeight: 500, marginBottom: 24 }}>Scoring invoice history and calculating risk</div>
          {progressBar("2s")}
        </div>
      )}
        ...
        <div onClick={() => setStage("connect")} style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, background: "rgba(15,23,42,0.55)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, fontFamily: "Roboto, Arial, sans-serif" }}>
          <div onClick={e => e.stopPropagation()} style={{ width: 448, background: "#FFFFFF", borderRadius: 8, boxShadow: "0 8px 28px rgba(0,0,0,0.28)", overflow: "hidden", animation: "fadeUp 0.2s ease-out" }}>
            ...
            <div style={{ padding: "40px 40px 32px", textAlign: "center" }}>
              ...
              <div style={{ fontSize: 15, color: "#5F6368", marginBottom: 24 }}>to continue to <span style={{ color: "#1A73E8" }}>Phoresight Clyintel</span></div>
                ...
                {googleAccounts.map((acct, i) => (
                  <div key={acct.email} onClick={() => handleAccountPick(acct)} style={{ display: "flex", alignItems: "center", gap: 14, padding: "14px 16px", borderBottom: i < googleAccounts.length ? "1px solid #DADCE0" : "none", cursor: "pointer", transition: "background 0.15s" }} onMouseEnter={e => e.currentTarget.style.background = "#F8F9FA"} onMouseLeave={e => e.currentTarget.style.background = "#FFFFFF"}>
                    <div style={{ width: 32, height: 32, borderRadius: "50%", background: acct.color, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                      <span style={{ fontSize: 15, fontWeight: 500, color: "#FFFFFF" }}>{acct.initial}</span>
                    </div>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontSize: 15, color: "#202124" }}>{acct.name}</div>
                      <div style={{ fontSize: 14, color: "#5F6368", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{acct.email}</div>
                    </div>
                  </div>
                ))}
            ...
            <div style={{ padding: "32px 40px 28px" }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 16 }}>
                <div style={{ width: 36, height: 36, borderRadius: 8, background: C.navy, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                  <span style={{ fontSize: 16, fontWeight: 700, color: "#FFFFFF", letterSpacing: "-0.5px" }}>P</span>
                </div>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 14px", background: "#F8F9FA", borderRadius: 8, marginBottom: 20 }}>
                ...
                <div style={{ fontSize: 15, color: "#202124" }}>{selectedAccount.email}</div>
              </div>
              ...
              <div style={{ display: "flex", alignItems: "flex-start", gap: 12, padding: "10px 0", borderTop: "1px solid #E8EAED" }}>
                ...
                <div style={{ fontSize: 15, color: "#3C4043", lineHeight: 1.5 }}>See and download all your Google Drive files</div>
              </div>
              <div style={{ display: "flex", alignItems: "flex-start", gap: 12, padding: "10px 0", borderTop: "1px solid #E8EAED" }}>
                ...
                <div style={{ fontSize: 15, color: "#3C4043", lineHeight: 1.5 }}>See your primary Google Account email address and personal info</div>
              </div>
      ...
      {stage === "drive_folders" && (
        <>
          ...
          <div style={{ maxWidth: 680, margin: "0 auto" }}>
            ...
            <div style={{ fontSize: 15, color: C.textMid, fontWeight: 500, marginBottom: 20 }}>Choose the folder containing your invoice CSVs.</div>
            <div style={{ border: `1px solid ${C.border}`, borderRadius: 10, overflow: "hidden" }}>
              ...
              {driveFolders.map((f, i) => (
                <div key={f.name} onClick={() => handleFolderPick(f)} style={{ display: "grid", gridTemplateColumns: "1fr 110px 140px 24px", gap: 16, padding: "14px 16px", alignItems: "center", borderBottom: i < driveFolders.length - 1 ? `1px solid ${C.border}` : "none", cursor: "pointer", transition: "background 0.15s" }} onMouseEnter={e => e.currentTarget.style.background = C.blueBg} onMouseLeave={e => e.currentTarget.style.background = "transparent">
                  ...
                  <div style={{ fontSize: 15, color: C.textMid, fontWeight: 500 }}>{f.modified}</div>
      ...
      {stage === "drive_files" && (
        <>
          ...
          <div style={{ maxWidth: 680, margin: "0 auto" }}>
            ...
            <div style={{ fontSize: 15, color: C.textMid, fontWeight: 500, marginBottom: 20 }}>{driveFiles.length} CSV files in <span style={{ fontWeight: 500, color: C.text }}>{selectedFolder?.name}</span></div>
            <div style={{ border: `1px solid ${C.border}`, borderRadius: 10, overflow: "hidden" }}>
      ...
      {stage === "csv_uploading" && (
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: 360 }}>
          <div style={{ fontSize: 14, color: C.textDim, fontFamily: C.mono, fontWeight: 500, marginBottom: 6 }}>{selectedFile?.name || "invoices.csv"}</div>
      ...
      {stage === "csv_done" && (
        <div style={{ maxWidth: 560, margin: "40px auto 0", textAlign: "center" }}>
          ...
          <div style={{ fontSize: 15, color: C.textMid, marginBottom: 4, fontWeight: 500 }}>
            <span style={{ fontFamily: C.mono, color: C.text, fontWeight: 500 }}>{selectedFile?.name}</span>
          </div>
          <div style={{ fontSize: 14, color: C.textMid, marginBottom: 28, fontWeight: 500 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            ...
            <button onClick={() => router.push("/")} style={{ padding: "12px 20px", fontSize: 15, fontWeight: 600, color: C.text, background: "#FFFFFF", border: `1px solid ${C.border}`, borderRadius: 8, cursor: "pointer" }} onMouseEnter={e => e.currentTarget.style.borderColor = C.blue} onMouseLeave={e => e.currentTarget.style.borderColor = C.border}>
        ...
        <>
          ...
          <div style={{ maxWidth: 520, background: C.card, border: `1px solid ${C.border}`, borderRadius: 12, padding: "28px 32px" }}>
            ...
            <div style={{ display: "flex", justifyContent: "flex-end", gap: 10, paddingTop: 8, borderTop: `1px solid ${C.border}` }}>
              ...
              <button onClick={() => { setManualSubmitted(true); setTimeout(() => router.push("/"), 1800); }} style={{ padding: "9px 18px", fontSize: 14, fontWeight: 600, color: "#FFFFFF", background: C.blue, border: "none", borderRadius: 6, cursor: "pointer" }} onMouseEnter={e => e.currentTarget.style.opacity = "0.88"} onMouseLeave={e => e.currentTarget.style.opacity = "1"}>Save Invoice</button>
            </div>
          </div>
        </>
      )}
